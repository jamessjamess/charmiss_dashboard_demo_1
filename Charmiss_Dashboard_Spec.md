...
| ความครอบคลุม | Active Stores/Partners, Retention | ไม่มี (**แก้ 2026-08-06:** Active Shop Listings ถูกตัดออกทั้งระบบ — ตั้งอยู่บนสมมติฐาน "หลาย Shop ต่อ Platform" ที่ไม่ตรงกับความจริง Charmiss มี 1 Shop ต่อ Platform เท่านั้น — Ecom ไม่มี Metric มุม Coverage ในโมเดลนี้) |
...
| Net Sales vs Target by Channel | **แก้ 2026-08-06 (redesign):** เดิมชื่อ "Net Sales: Last Year vs This Year vs Target" — 1 แท่งต่อ Channel (MT/TT/Ecom, สี Channel เดิม ไม่ใช่สีเดียวคงที่แบบก่อน) แสดงเฉพาะ This Year — Target ไม่มีแท่งแยกแล้ว แทนที่ด้วย Tick Mark บนแท่ง (เขียว = Hit/แดง = Miss เทียบ Target เดือนนั้น) Data Label เหลือแค่ Net Sales ฿ + Attainment % (สีตาม Hit/Miss) Last Year/Growth YoY/Gap to Target ย้ายไปอยู่ใน Hover Tooltip ต่อแท่งแทน (ไม่แสดงตลอดเวลาแล้ว) เหตุผล: Revenue Trend + Channel Growth Comparison (YoY) ตอบคำถาม "โตจากปีก่อนกี่ %" อยู่แล้ว กราฟนี้จึงเหลือหน้าที่เดียวคือ "เข้าเป้าไหมต่อ Channel" — เพิ่ม Insight Caption ใต้กราฟสรุป Hit/Miss รวม (เช่น "All 3 channels above target (Year to Date) — MT +฿582.1K · TT +฿687K · Ecom +฿330.6K") ปุ่ม "ดูเป็นตาราง" ยังแสดงตัวเลขละเอียดครบ 6 คอลัมน์เหมือนเดิม (Last Year, This Year, Growth YoY, Target, Attainment %, Diff from Target) |
...
| [ระดับที่เลือก] Growth Contribution | Waterfall — แทนที่ Category Share of Sales Over Time เดิม (ย้ายไป Full Width ด้านล่างแล้ว) แนวคิดเดียวกับ Growth Contribution Waterfall ของ Executive Outlook แต่เป็นระดับ Category/Sub-Category/Type/Series แทน Channel — "ปีก่อน" ต่อ Node ประมาณจากยอดขายปีนี้หารด้วย (1+%Growth YoY ของ Node นั้น) เพราะ Mock Data ไม่มี Time Series จริงระดับ Category (มีแต่ %Growth YoY เป็นค่าคงที่ต่อ Node) — คำนวณจาก `syncedAll` (Node Set เดียวกับที่ Sync Level+Channel Filter) เสมอ เพื่อให้ Start+ผลรวม Delta ตรงกับ End เป๊ะไม่ว่าจะกรอง Channel หรือไม่ — จำกัด Top 8 by \|Contribution\| + Focus Pin ที่เหลือรวมเป็นแท่ง "Other (N)" — ใช้ `opts.rotateLabels:90` ของ `buildWaterfallSVG` (แนวตั้งเต็ม, จำเป็นเพราะจำนวนแท่งสูงสุดถึง 10 ตัว Label แนวนอนชนกัน) ⚠️ **Gotcha ที่เจอจริง:** มุมลบ (`rotate(-35...)`) ใน SVG (แกน Y ชี้ลง) กวาด Label ลง ไม่ใช่ขึ้น — ใส่มุมลบผิดจะทำให้ Label ทะลุไปทับ Insight Callout ใต้กราฟ (SVG ไม่ Clip Overflow โดย Default) ต้องใช้มุม**บวก**เสมอเพื่อกวาด Label ขึ้น-ซ้าย |
...
- **แก้ 2026-08-06 (รอบสาม, Feedback):** "Return Rate (CN)" Trend — ผู้ใช้รายงานว่ากราฟ "ไม่เต็มกรอบ" (เหลือพื้นที่ว่างขวามือ) — เดิมพล็อตด้วย `curYearIdx` เต็มปี (Jan–Dec) แล้วปล่อยให้เดือนที่ยังไม่ปิดงวดเป็น `null` (ตั้งใจไว้แต่แรกว่า Line Chart หยุดวาดตรง `null` เองโดยไม่ต้องใส่ค่าหลอก ไม่ถือว่า Misleading) แต่ผลข้างเคียงคือพื้นที่ Canvas ครึ่งขวาว่างเปล่าเสมอ ดูไม่เต็มกรอบจริง — เปลี่ยนไปใช้ `closedIdx` (Jan ถึง TODAY เท่านั้น ตัวเดียวกับที่ Target vs Actual Bar Chart ด้านบนใช้อยู่แล้ว) แทน ทำให้เส้นกราฟยืดเต็มความกว้าง Container เสมอ (ยังคง Responsive ปรกติ เพราะ `buildLineChartSVG` Scale แกน X ตาม Label Array ที่ส่งเข้าไปอัตโนมัติอยู่แล้ว ไม่ใช่ Fix ขนาด) — Return/Cancellation Rate Trend ของ ECOM Breakdown มี Pattern เดียวกันเป๊ะ แก้พร้อมกันตอนปรับ ECOM Breakdown รอบใหญ่ (ดู §5.5)
...
**Data Model ที่เพิ่มเข้ามาสำหรับหน้านี้ (`storeOpenedMonth`):** `a.storeOpenedMonth` (Array ขนาดเท่า `storeCount`, `null` = เปิดมาก่อนหน้าต่างข้อมูล 36 เดือน, หรือ Index เดือนใน 12 เดือนล่าสุด = เพิ่งเปิด) และ `a.newStoreWeightShare` (สัดส่วน Weight ของสาขาที่ Tag ว่า "ใหม่") ต่อ Key Account ที่มี `storeCount` — สาขา "ใหม่" สุ่มเลือก**แบบสุ่มล้วน ไม่ใช่เรียง Weight น้อยไปมาก** (เรียง Weight ก่อนจะ Tag สาขาเกินสัดส่วนจริง เพราะสาขา Long-tail มี Weight น้อยมาก ต้องใช้จำนวนมากถึงจะรวมได้ถึง Target) — SSSG = `((1+growthYoY/100)*(1-newStoreWeightShare)-1)*100`, `NEW_STORE_TARGET_SHARE` เป็นค่า Illustrative ต่อ Account (WATSONS 2.5%, EVEANDBOY 12%, CJ Express 8%, Beautrium 5%, Multy 1.5%) — ⚠️ **Gotcha ที่เจอจริง (Class ของ Bug ที่ควรระวังเวลาผูก 2 ระบบเข้าด้วยกัน):** `a.newStoreWeightShare` เคย**ไม่ใช่ค่า Target ที่ประกาศไว้ แต่เป็นค่า `cum` จริงจากการ Tag สาขาทั้งสาขา** — สำหรับ Account ที่มีสาขาน้อยมาก (Multy 2 สาขา) การ Tag แค่ 1 สาขาอาจกิน Weight 50%+ ของ Account ทันที ทำให้ Target เล็กๆ (1.5%) กลับได้ `cum` จริงสูงถึง ~80%+ (SSSG พุ่งไปที่ -82% จาก Total Growth -6%) — แก้โดยแยก 2 เรื่องออกจากกัน: `a.newStoreWeightShare` ใช้ค่า `target` ที่ประกาศตรงๆ ส่วนการ Tag สาขาจริง (`storeOpenedMonth`) ยังใช้ Mechanism เดิมแต่จำกัดผลไว้แค่ KPI "New Stores Opened"/ตาราง ไม่กระทบสูตร SSSG อีกต่อไป — **บทเรียน:** เวลามี Field 2 ตัวที่ "ควรจะ" มีค่าตรงกัน (Declared Target vs Derived Actual) ต้องเช็คว่าฝั่งที่ถูกใช้จริงในสูตรคือตัวไหน อย่าสมมติว่าตรงกันเสมอโดยไม่ตรวจ
...
**แก้ 2026-08-06 (รอบสาม, Feedback — CN Record intermittent rendering glitch):** ผู้ใช้รายงาน (ซ้ำ 3 Session) ว่าข้อความจาก KPI Card ด้านบน (Target Achievement/This Month at a Glance) หลุดมาซ้อนทับ CN Record ที่แถว TT-1018/TT-1041 เป็นครั้งคราว ไม่เกิดทุกครั้ง — Reproduce โดยสลับ Rep/Year/Month ทุก Combination (5 Rep × 6 เดือน = 30 Combo), Jump-scroll ไปตำแหน่งต่างๆ, และ Simulate Fast Continuous Scroll ผ่าน `requestAnimationFrame` ข้าม 3 เท่าของความสูงหน้าเว็บ พร้อมตรวจ DOM ทุกจุด (Duplicate ID, Stray Fixed/Absolute Element ที่ทับ `#cnTable`, `.card-focused` ที่หลุดค้าง) — **ไม่พบข้อบกพร่องระดับ DOM/State เลยแม้แต่จุดเดียว ไม่มี Error ใน Console** ตัดสาเหตุระดับ Application Logic ออกไปได้ค่อนข้างมั่นใจ — จุดที่ผิดปกติที่สุดในหน้านี้คือ `.l2-mini-nav` (Sticky Nav ของ Anchor Pill, `position:sticky`) ที่วางอยู่ระหว่าง Section KPI Hero (บน) กับ Section Credit Notes (ล่าง) พอดี — ตรงกับ Chromium Bug Class ที่รู้จักกันดี: Sticky Element บางครั้ง Compositor ไม่ Re-composite Layer ให้สะอาดระหว่าง Scroll เร็วจริง (ต่างจาก Scroll ที่ Script สั่งตรงๆซึ่งมักไม่ Trigger เพราะไม่มี Frame-drop จริงแบบ Trackpad/Mouse Wheel ของ User จริง) ทำให้ Content ที่เคย Paint ไว้ตรง Layer Boundary หลุดค้างซ้อนกับ Content ด้านล่างจนกว่าจะมี Repaint Trigger ใหม่ (Hover/Click/Scroll ต่อ) — ตรงกับทุก Symptom ที่รายงาน (Intermittent ตาม Timing ไม่ใช่ตาม State ข้อมูล, ตำแหน่งเดิมซ้ำๆ เพราะผูกกับ Scroll Position ไม่ใช่ค่าข้อมูล, เกิดข้าม Session เพราะเป็น Browser/GPU-level ไม่ใช่ Code-level) — **แก้โดยเพิ่ม `will-change:transform` ให้ `.l2-mini-nav`** (`tt-shared.css`) บังคับให้ Sticky Nav ได้ Compositor Layer ของตัวเอง กัน Repaint Region ของมันไม่ให้ไปกระทบ Content ด้านล่างได้อีก — เป็น Fix มาตรฐานสำหรับ Sticky-scroll Ghosting Bug Class นี้ ความเสี่ยงต่ำ (CSS-only, กระทบแค่ Nav ตัวเดียว) — **หมายเหตุสำคัญ:** เพราะเป็น GPU Paint-timing Bug ไม่ใช่ Bug ที่ Reproduce ได้แน่นอนผ่าน Static Analysis จึงไม่สามารถยืนยัน "แก้หายขาด 100%" ได้จาก Session นี้ — ถ้า Recur อีกหลัง Deploy Fix นี้ ให้ถือว่า Diagnosis (Sticky-scroll Interaction) ยังถูกอยู่ แค่ Fix นี้ยังไม่พอ (อาจต้องเพิ่ม `contain:paint` ให้ `.card` ด้วย) ไม่ใช่กลับไปหา Bug ใน Application Code
...
**Executive Summary** (4 KPI Card — ตามหลักการเดียวกับ MT/TT คือไม่มีการ์ด Growth YoY/Growth MoM แยกต่างหาก เพราะซ้ำกับแถว MoM/YoY ในการ์ด Net Sales/Target Attainment): Net Sales (Ecom), Target Attainment %, Return/Cancellation Rate, AOV (Average Order Value) · Revenue Trend, Monthly YoY Growth % (Chart) · Sales by Platform (Donut — Lazada/Shopee/TikTok/Line Shop, สีตรงกับ `--lazada`/`--shopee`/`--tiktok`/`--lineshop` ใน `shared.css`) · Sales by Category (มี Product Level Toggle ครบ 4 ระดับ) — **แก้ 2026-08-06 (รอบสี่, Feedback):** ตัด **Conversion Rate** และ **Active Shop Listings** ออกทั้งคู่ — Conversion Rate แยกความแตกต่างต่อ Platform ได้จริงแค่ผ่าน Skew เชิงเล่าเรื่อง ไม่มีความหมายเชิงวิเคราะห์เพิ่มจาก AOV ที่มีอยู่แล้ว; Active Shop Listings ตั้งอยู่บนสมมติฐาน "หลาย Shop ต่อ Platform" เดียวกับที่ตัดออกจาก Breakdown ไปแล้วก่อนหน้า (Charmiss มี 1 Shop ต่อ Platform จริง) — ลบ `ecomConvSeries`/`ecomShopListingsSeries` Generation ทิ้งทั้งคู่ (ยืนยันแล้วว่าไม่มี Code ใดใน 2 ไฟล์ ECOM เรียก `rand()` ต่อจากจุดนี้อีก จึงลบได้โดยไม่กระทบลำดับ rand())

`ecomPlatformStats` (Copy ไว้ทั้งใน Executive Summary และ Breakdown) เปลี่ยนจาก `{aov, conv, cancelFactor}` (ตัวเลขสัมบูรณ์คงที่) เป็น **`{aovIndex, cancelFactor}`** (ตัวคูณ, Weighted-average ≈1.0 ข้าม 4 Platform — `convIndex` ถูกลบไปพร้อม Conversion Rate ด้านบน) — เหตุผลอยู่ที่ Breakdown ด้านล่าง (Period-sensitivity)

**แก้ 2026-08-06 (รอบสาม, Feedback):** เพิ่ม `<nav class="section-nav">` (Executive Summary / Trends & Mix) ให้ตรงกับ Pattern ที่ MT ("Executive Summary | Key Account Portfolio") และ TT ("Executive Summary | Trends & Mix") มีอยู่แล้ว — เดิม ECOM Executive Summary ไม่มี Jump Navigation เลย — เพิ่ม `id="sec-exec-summary"` ให้ `.section-label` เดิม (KPI Zone) และเพิ่ม `.section-label` ใหม่ `id="sec-trends-mix"` ก่อน Chart-grid ของ Revenue Trend/Monthly YoY Growth% — ใช้ชื่อ "Trends & Mix" เหมือน TT (ไม่ใช่ "Key Account Portfolio" แบบ MT) เพราะ ECOM ไม่มี Entity "Partner/Key Account" (Ecom มี Platform เท่านั้น — ดู Root Cause เดียวกันที่อธิบายไว้ใน §5.5 ด้านบน) เนื้อหาโซนนี้ (Revenue Trend, Monthly YoY Growth%, Sales by Platform, Sales by Category, Net Sales: Last Year vs This Year vs Target, Platform Growth Contribution) ใกล้เคียงกับที่ TT จัดไว้ใต้ "Trends & Mix" มากกว่า — ไฟล์นี้ใช้ `shared.css`/`shared.js` อยู่แล้ว (`.section-nav` มีอยู่แล้วไม่ต้อง Port ข้าม Engine เหมือนที่ TT ต้องทำ)

ตรวจสอบ Feedback อีกจุด: "Sales by Category — Breakdown" Chart Title ที่ ECOM Breakdown → Product Coverage ที่ผู้ใช้รายงานว่ามี Typo "Categery" — ตรวจสอบไฟล์จริงแล้วไม่พบ Typo นี้ (Title ปัจจุบันคือ "Sales by Category" เฉยๆ ไม่มีคำว่า "— Breakdown" ต่อท้ายด้วย) — ไม่ได้แก้อะไรเพราะไม่มี Bug อยู่จริงในโค้ดปัจจุบัน (อาจมาจาก Version ก่อนหน้าที่ถูกปรับไปแล้วระหว่าง ECOM Breakdown ปรับใหญ่รอบก่อน)

**แก้ 2026-08-06 (Feedback):** เพิ่มการ์ด **"Net Sales: Last Year vs This Year vs Target"** ใน Executive Summary — Grouped Bar 3 แท่งต่อ Platform (Last Year / This Year / Target), Port มาจากการ์ดชื่อเดียวกันใน `sales_overview.html` (มิติเดิมคือ MT/TT/Ecom เปลี่ยนเป็น 4 Platform ของ Ecom เอง) — ใช้ `ecomChannels[i].actual`/`.target` จริงตาม Period Filter ที่เลือก (`sumRange` เทียบช่วงปีก่อนหน้าแบบเดียวกับ Platform Growth Contribution Waterfall ที่มีอยู่แล้ว จึงคำนวณได้จริงไม่ต้องเพิ่ม Mock Data ใหม่) — Value Label ต่อแท่งปรับ Responsive ตาม Bar Width (`buildGroupedBarSVG`'s `valueLabelFormatter`): แท่งกว้างพอโชว์ครบ (฿Value + Growth YoY/Attainment %+Diff from Target), แท่งแคบ (<28px, เกิดขึ้นได้เมื่อมี 4 Platform ในความกว้างการ์ดมาตรฐาน) โชว์แค่ Growth YoY บนแท่ง This Year เพื่อไม่ให้ Label ทับกัน — มีปุ่ม Toolbar ครบ (Table View/Download SVG/Expand) ตาราง Column: Platform, Last Year, This Year, Growth YoY, Target, Attainment %, Diff from Target — ทดสอบแล้วว่าตอบสนอง Period Filter ถูกต้อง (ค่าเปลี่ยนจริงเมื่อสลับ Year to Date → Last Quarter) — **ตำแหน่ง (แก้ 2026-08-06 รอบสอง, Feedback):** ย้ายจากเดิม (ระหว่าง KPI Grid กับ Revenue Trend/Monthly YoY Growth%) ไปอยู่**เหนือ Platform Growth Contribution** แทน (หลัง Sales by Platform/Sales by Category) เพราะทั้งคู่เล่าเรื่อง "This Year vs Last Year ต่อ Platform" คนละมุมมองแต่ใกล้เคียงกัน (Bar เทียบ Value ตรงๆ vs Waterfall เทียบ Contribution สะสม) — วางติดกันอ่านง่ายกว่า
...
| Portfolio Quality | Return/Cancellation Rate Trend (รายเดือน, เส้นประ Threshold 8%) จับคู่กับ **Return/Cancellation Rate by Platform** | Return/Cancellation by Platform = Ecom-wide Period Average (จริง, จาก `ecomCancelRate`) × `cancelFactor` ต่อ Platform — **แก้ 2026-08-06 (รอบสี่, Feedback):** เปลี่ยนจาก `buildHBarCompareSVG` (SVG Bar Chart) เป็น `.cat-bars`/`.cat-ref-line` Pattern เดียวกับ MT's "High Return Rate Partner" เป๊ะ (เส้นประ Threshold ในแนวนอนของแท่ง + ตัวเลขฝังในแท่งสี แทนกราฟ SVG แยก) เพื่อให้ 2 Channel หน้าตาเหมือนกัน |
...
| **Platform Performance** (Section ใหม่ แทน "Shop Performance") | Monthly Sales by Platform (Full-width Trend Line 36 เดือน, 4 เส้นจริงจาก `ecomChannels[i].actual`) | Real Time Series ต่อ Platform อยู่แล้ว (ไม่ใช่สัดส่วนคงที่ของยอดรวม) มีแค่ 4 เส้นจึงไม่ต้อง Dim เส้นแบบ MT's 9-Partner Version |
...
| Platform Performance | **Sales by Platform** จับคู่กับ **Platform Portfolio Matrix** | Sales by Platform = Ranked Bar เหมือน MT's "Sales by Partner" (4 แถวเท่านั้น ไม่ต้อง Cap Top-N) — Platform Portfolio Matrix = Adapted BCG Matrix เหมือน Category/Partner Portfolio Matrix (X=ยอดขาย, Y=Growth YoY, ใช้ค่าเดียวกับ YoY Growth by Platform ด้านบนเพื่อความสอดคล้อง) — **แก้ 2026-08-06 (รอบสี่, Feedback):** Sales by Platform (แค่ 4 แถว) เคยดูเหมือน "ไม่เต็มกรอบ" เพราะ CSS Grid ยืดการ์ดให้สูงเท่า Platform Portfolio Matrix (280px Canvas) แต่แถวสั้นๆ กระจุกอยู่บนสุด เหลือพื้นที่ว่างล่างการ์ด — แก้ด้วย Class ใหม่ `.card-vcenter-fill` (`shared.css`, Opt-in เฉพาะการ์ดนี้ ไม่แก้ `.card` รวมทั้งระบบเพราะเปลี่ยน Margin-collapse Behavior ของทุกการ์ดในระบบ ความเสี่ยงสูงเกินไป) ทำให้ `.cat-bars` ขยายเต็มพื้นที่ที่เหลือแล้ว Center แนวตั้ง |
| Platform Performance | Platform Share of Sales Over Time จับคู่กับ **AOV by Platform** | **แก้ 2026-08-06 (รอบสี่, Feedback):** ตัด Conversion Rate by Platform ออก (เหตุผลเดียวกับ Executive Summary ด้านบน) — Platform Share of Sales Over Time (เดิม Full-width เดี่ยว) ย้ายมา Pair กับ AOV by Platform แทนที่ช่องที่ Conversion Rate เคยอยู่ |
| Product Coverage | Sales by Category, Category Share of Sales Over Time, Platform × Category | **แก้ 2026-08-06 (รอบสี่, Feedback):** Sales by Category เพิ่ม Product Level Toggle (Category/Sub-Category/Type/Series, Copy `CATEGORY_TREE`/`enumerateEcomCatLevel()` มาจาก Executive Summary) — Category Share of Sales Over Time และ Platform × Category Sync ตาม Level เดียวกัน (Pattern เดียวกับ MT's Sales by Category/Category Share/Partner × Category) — Category Share of Sales Over Time ย้ายจาก `buildStackedAreaSVG`+Static%จริง เป็น `buildStreamAreaSVG`+ค่า ฿ จริงจาก `groupActual.Ecom` (จำเป็นเพราะ Static % ไม่มีทางแทน Node ที่ระดับลึกกว่า Category ได้) — **Category Portfolio Matrix ไม่ Sync** ยังคงอยู่ที่ระดับ Category เสมอ (เหมือน MT — Mock Growth% มีแค่ระดับ Category เท่านั้น) |
| Product Coverage | ~~Top 5 / Bottom 5 SKU by Revenue~~ (ตัดออก) แทนที่ด้วย **Platform × Category** (ล่างสุด) | ตัด SKU Table ทั้งคู่ทิ้งทั้งหมด (ลบ `skus[]`/`SKU_COUNT` Mock Data Block ด้วย เพราะไม่มีจุดใช้อื่นแล้ว) แทนด้วย Heatmap Cross-tab แถว=4 Platform × คอลัมน์=Top 8 Node ตาม Level ที่เลือก (เหมือน MT's Cap, ต่างจากเดิมที่ Fix 6 Category เสมอ) — Pattern เดียวกับ MT Breakdown's "Partner × Category" แต่สลับแถวจาก Key Account เป็น Platform (Ecom ไม่มี Entity "Partner") |
...
Metric ที่ไม่มีใน MT/TT: ไม่มี AR% (ลูกค้าจ่ายผ่าน Platform โดยตรง ไม่มี Ledger ลูกหนี้แบบ MT/TT), ใช้ Return/Cancellation Rate แทน Return Rate (CN) เดี่ยวๆ, เพิ่ม AOV ทั้งใน Executive Summary (Company-wide) และ Breakdown (แยกตาม Platform, Period-sensitive) — **Conversion Rate ถูกตัดออกทั้งระบบ** (2026-08-06 รอบสี่ — ไม่มีความหมายเชิงวิเคราะห์เพิ่มจาก AOV ที่มีอยู่แล้ว) — Ads Spend ROI ที่เคยพิจารณาไว้ในดราฟต์แรกถูกตัดออกจาก Scope ตั้งแต่รอบแรก
...
**Executive Summary** (7 KPI Card): Gross Profit, Gross Margin %, Net Profit, Net Margin %, Profit Target Attainment % (Net Profit เทียบเป้าหมายที่ตั้งจาก Net Sales Target × 22% Net Margin สมมติฐาน), **Full Year Profit Forecast** (เพิ่ม 2026-08 — Run-rate เดียวกับ Sales Overview's Full Year Forecast: (Net Profit Calendar-YTD ÷ เดือนที่ผ่านไป) × 12 ÷ Full Year Target × 100, ใช้ `buildTrajectorySparklineSVG` เส้นทึบ+เส้นประ — **ไม่มีแถว MoM/YoY** ตามข้อยกเว้นเดียวกับ Sales Overview เพราะเป็นค่าพยากรณ์เต็มปี ไม่ใช่ค่าจริงรายเดือน), OPEX Ratio % (Delta กลับทิศเหมือน Return/Cancellation Rate — ยิ่งต่ำยิ่งดี) · Profit Trend (เพิ่มเส้น **Last Year Net Profit** 2026-08 — เทียบ Gross Profit/Net Profit/Target/ปีก่อนในกราฟเดียว), Margin Trend (Chart, รายเดือน) · P&L Waterfall (Net Sales → COGS → Gross Profit → Trade Spend → Logistics → Marketing/Ads → SG&A → Net Profit) · **Income Statement** (เพิ่ม 2026-08 — Table ต่อจาก Waterfall ทันที: แถวเดียวกัน 8 แถวตาม Waterfall Order, คอลัมน์ This Period/Last Year/Diff %/Sparkline รายเดือน — **คำนวณจากตัวแปร Sum ชุดเดียวกับ Waterfall เป๊ะ** (`netSalesSum`/`cogsSum`/.../`netProfitSum`) เพื่อการันตีตัวเลขตรงกัน 100% ทุก Row ไม่ว่า Period Filter จะเป็นค่าไหน — เจตนา Design: Waterfall กับ Income Statement เป็น Data Source เดียวกัน แสดงคนละรูปแบบ ถ้าตัวเลขไม่ตรงกันจะทำลาย Credibility ทั้งคู่) · Margin by Channel, Margin by Category (Grouped Bar เทียบ Gross %/Net %) · Ranking by Channel (Table)

**ทำไมไม่มี "Revenue" Sub-module ใน P&L:** Sales Overview ครอบคลุม Revenue Breakdown อยู่แล้วอย่างละเอียด — เพิ่มอีกใน P&L จะซ้ำปัญหา "สองที่พูดเรื่องเดียวกัน" แบบเดียวกับที่เคยแก้ระหว่าง MT Executive Summary/MT Store — ถ้า P&L ต้องอ้างอิง Revenue Breakdown ให้ลิงก์ไปที่ Sales Overview แทนสร้างกราฟซ้ำ (ดู Subtitle ของ Income Statement Table ด้านบน)

**Breakdown (Margin/Cost Structure เชิงลึก — 2 Section):**
| Section | Metric/Chart |
|---|---|
...
Filter Bar ของ Breakdown: Period, **Channel** (All/MT/TT/Ecom — scope เฉพาะ Zone Category Profitability), Category (ปัจจุบันใช้งานได้แค่ "All Categories" เหมือน Module อื่น) — ไม่มี Filter รายบุคคล เพราะ P&L เป็นมุมมอง Company/Channel-level ไม่มี Owner ต่อบุคคล

**Cost Structure (`pnl_cost_structure.html`, ไฟล์ที่ 3 — เพิ่ม 2026-08):** เดิม Breakdown มี Zone "Cost Structure Over Time" (OPEX Trend + Marketing & Ads Spend by Channel) — **ย้ายทั้ง Zone ออกมาเป็นแท็บใหม่** พร้อมขยายเป็น 3 Section, ไม่ใช่แค่ย้ายเฉยๆ:
| Section | Metric/Chart |
|---|---|
| Cost Efficiency | OPEX Ratio % KPI (ตัวเดียวกับ Executive Summary), Cost-to-Sales Ratio Table (Trade Spend/Logistics/Marketing/SG&A ÷ Net Sales — This Period/Last Year/Diff pp) |
| Cost by Channel & Partner | OPEX Trend (ย้ายจาก Breakdown), **Operating Expense Ratio Trend by Channel** (ใหม่ — OPEX Ratio % รายเดือนต่อ MT/TT/Ecom, กลับทิศจาก Net Margin % Trend by Channel ของ Breakdown), Marketing & Ads Spend by Channel (ย้ายจาก Breakdown), **Trade Spend by Partner (MT only)** — **Placeholder ล้วนๆ ไม่มีตัวเลขจริง**: ยังไม่มี Data Source ระดับ Partner สำหรับ Trade Spend (ต้องใช้ Negotiated Deal Terms ต่อคู่ค้า ไม่ใช่สัดส่วนยอดขาย) การ์ดนี้แสดง Track เปล่า + Caveat Text ตรงๆ แทนตัวเลขแต่งขึ้น |
| Cost Efficiency Signal | "For Every ฿1 of Revenue" Donut (COGS/OPEX/Net Profit ตามสัดส่วนจริง) + Callout อัตโนมัติชี้ Cost Line ที่โต YoY เร็วกว่า Net Sales |

**Standing Guardrail:** ห้ามสร้าง Trade Spend/Cost Breakdown แยกอีกใน MT/TT/ECOM Breakdown — P&L (โดยเฉพาะแท็บ Cost Structure นี้) เป็น Single Source of Truth สำหรับมิติ Cost/Trade Spend ทั้งระบบ

ทั้ง 3 แท็บมี Anchor Pill Nav (`<nav class="section-nav">`) ยกเว้น Executive Summary (เป็น Zone เดียวต่อเนื่อง ไม่แบ่ง Sub-section ให้ Jump)
...
**แก้ 2026-08-06 (รอบสาม, Feedback — Root Cause ตัวจริงของ Label ชนที่เหลือ):** ผู้ใช้รายงานอีกครั้งว่า Label ยังชนกันอยู่ ครั้งนี้สืบพบ Root Cause ที่แท้จริง: **TT Overview — Breakdown's "Category Portfolio Matrix" ไม่ได้ใช้ `buildScatterSVG` ของ `shared.js` เลย** — TT ทั้งไฟล์ใช้ Engine แยกของตัวเอง (`tt-shared.js`, ไม่ import `shared.js`, ดู §6.3) มีฟังก์ชัน Scatter ของตัวเองชื่อ `renderScatterMatrix` ซึ่งไม่มี Collision-avoidance เลยตั้งแต่สร้าง (Label Fix ตำแหน่งเดียวเสมอที่ `cy-11` เหนือจุด ไม่เช็คจุดอื่นเลย) — ทุกครั้งที่แก้ `buildScatterSVG` ก่อนหน้านี้ (รวม 2 รอบบน) จึงไม่มีผลกับ TT Breakdown เลยแม้แต่นิดเดียว เพราะเป็นคนละฟังก์ชัน คนละไฟล์ — นี่คือสาเหตุที่ "แก้แล้วแต่ปัญหากลับมาใหม่ที่อื่น" ตามที่ผู้ใช้สังเกต — **แก้โดย Port Algorithm เดียวกัน (8 ตำแหน่งผู้สมัคร + Real Bounding Box จาก `canvas.measureText()`) เข้าไปใน `renderScatterMatrix` โดยตรง** (เพิ่ม Helper ใหม่ `ttMeasureTextWidth()` ในไฟล์เดียวกัน เพราะ TT ไม่มี Access ไปยัง `measureTextWidth()` ของ `shared.js`) รวมถึงลงทะเบียนกล่องของ Quadrant Watermark Label ทั้ง 4 มุมเป็น "จุดที่ถูกจองแล้ว" ก่อน Loop วาง Label จริง เพื่อไม่ให้ Label ทับ Watermark ด้วย — ทดสอบยืนยันแล้วทั้ง 4 หน้าที่มี Chart ประเภทนี้ (Product Analysis, MT Breakdown, ECOM Breakdown ใช้ `buildScatterSVG` เดิม; TT Breakdown ใช้ `renderScatterMatrix` ที่แก้ใหม่) ด้วยการอ่าน Bounding Box จริงของทุก Label ผ่าน `getBBox()` เทียบกันทุกคู่ — ไม่พบการชนแม้แต่คู่เดียว รวมถึงทดสอบ TT ที่ระดับ Series (ลึกสุด, 14 Node) ซึ่งเป็นจุดที่ชนกันได้ง่ายที่สุดก็ผ่าน — **บทเรียน:** เวลามี Component "ใช้ซ้ำหลายหน้า" ต้องเช็คก่อนว่าทุกหน้าเรียก Function เดียวกันจริงหรือไม่ ไม่ใช่แค่ดูว่า Feature หน้าตาเหมือนกัน — TT engine แยกจาก MT/ECOM/Sales Overview มาตั้งแต่แรกอยู่แล้ว (ดู §6.3) จึงมีความเสี่ยงเฉพาะจุดนี้เสมอ
...
| `nav-menu.js` | `MODULE_MAP` — กำหนดลำดับ/รายชื่อ Module ใน Nav Dropdown และในหน้า `index.html` (Directory listing) พร้อมกัน — แก้ไฟล์นี้กระทบทั้ง 2 จุดพร้อมกันเสมอ — **แก้ 2026-08-05**: เพิ่ม Item `mt-store` (label "Store", href `module_mt_store.html`) เข้าไปในกลุ่ม MT Overview — ต้องเพิ่ม Tab Link คู่กันในทั้ง 3 ไฟล์ของ MT Overview ด้วย (`<nav class="tabs">` ของ Executive Summary/Breakdown/Store) ไม่ใช่แค่ตรงนี้ — **แก้ 2026-08 (P&L Cost Structure)**: เพิ่ม Item `pnl-cost` (label "Cost Structure", href `pnl_cost_structure.html`) เข้ากลุ่ม P&L Overview ด้วยหลักการเดียวกัน — เพิ่ม Tab Link คู่กันในทั้ง 3 ไฟล์ของ P&L Overview |
...
| ECOM → Executive Summary | `module_ecom_executive_summary.html` | Part 1 = copy จาก `sales_overview.html` (รวม `mtCNRate`/`ttCNRate`/`ecomCancelRate` Block — ไฟล์นี้ **ไม่ได้ใช้ `mtCNRate`/`ttCNRate` จริง** แค่ต้องคง Block ไว้ตำแหน่งเดิมเพื่อรักษาลำดับ rand()); **เป็นไฟล์ต้นทาง** ของ `ecomCategoryShares`, `ecomAOVSeries`, `ecomPlatformStats`, `CATEGORY_TREE`/`enumerateEcomCatLevel()` (Part 2) — `channelDefs` (Part 1) มี `LineShop` เป็น Entry สุดท้ายของกลุ่ม Ecom (ต้องแก้พร้อมกับทุกไฟล์ใน §6.2 ที่ก็อบปี้ Array นี้ — ดู §5.5), `ecomPlatformStats` = `{aovIndex,cancelFactor}` — **แก้ 2026-08-06 (รอบสี่):** ลบ `ecomConvSeries`/`ecomShopListingsSeries` Generation ทั้งคู่ (Conversion Rate/Active Shop Listings KPI ถูกตัด, ยืนยันแล้วว่าไม่มี `rand()` เรียกต่อจากจุดนี้ในไฟล์นี้อีกเลย ปลอดภัยที่จะลบ) และลบ `convIndex` ออกจาก `ecomPlatformStats` | Spec §5.5 |
| ECOM → Breakdown | `module_ecom_breakdown.html` | Part 1 = copy จาก `module_ecom_executive_summary.html` (รวม `mtCNRate`/`ttCNRate`/`ecomCancelRate` Block เช่นเดียวกัน — ไม่ได้ใช้ `mtCNRate`/`ttCNRate` จริงเช่นกัน); copy byte-for-byte `ecomAOVSeries`/`ecomPlatformStats`/`CATEGORY_TREE`/`enumerateEcomCatLevel()` จากไฟล์เดียวกัน (Level ตัวนี้ยังขับ `enumeratePlatformCatShare(key, depth)` ของ Platform × Category ในไฟล์นี้ด้วย) — แก้ Active Shop Listings (Executive Summary)/`ecomPlatformStats` ต้องแก้คู่ — ลบ `shops[]`/`PLATFORM_GROUPS`/`basesSum`/`skus[]`/`SKU_COUNT`/`ecomConvSeries`/`ecomShopListingsSeries` ทั้งหมด (Breakdown-only Mock Data ไม่มี Cross-file Sync Requirement นอกจาก `ecomConvSeries`/`ecomShopListingsSeries` ที่ลบพร้อมกับไฟล์ Executive Summary) — ดู §5.5 สำหรับรายละเอียด Section ที่ปรับใหม่ทั้งหมด | Spec §5.5, Review 1.4/2.2 |
...
| P&L → Executive Summary | `pnl_executive_summary.html` | Part 1 = copy จาก `sales_overview.html`; Part 2 ใช้ `mtCategoryShares`/`ecomCategoryShares` (copy จาก MT/ECOM Executive Summary) + `ttCategoryShares` (⚠️ สร้างขึ้นเองจาก Screenshot ใน Review doc ไม่ใช่ byte-for-byte เพราะไม่มีไฟล์ TT จริง) — **Income Statement Table** (เพิ่ม 2026-08) reuse ตัวแปร Sum ชุดเดียวกับ `waterfallSteps` ตรงๆ ไม่คำนวณซ้ำ | Spec §5.6, `Charmiss_Module_PnL_Spec.md` |
| P&L → Breakdown | `pnl_breakdown.html` | Part 1 = copy จาก `pnl_executive_summary.html` (ไม่ใช่จาก sales_overview ตรงๆ) — Part 2 "kept in sync with pnl_executive_summary.html" ตามคอมเมนต์ในไฟล์ — **แก้ 2026-08**: ลบ Zone "Cost Structure Over Time" (OPEX Trend/Marketing & Ads Spend by Channel) ทั้ง HTML+JS ทิ้ง ย้ายไป `pnl_cost_structure.html` แทน (เหลือ 2 Zone: Margin Quality by Channel, Category Profitability) | Spec §5.6, `Charmiss_Module_PnL_Spec.md` |
| P&L → Cost Structure (**ใหม่ 2026-08**) | `pnl_cost_structure.html` | Part 1+2 = copy จาก `pnl_breakdown.html` แบบ byte-for-byte (Data Model เดียวกันทั้งหมด ต้อง Sync `channelDefs`/`OPEX_RATES`/`categorySharesByChannel` คู่กันเสมอ) — เพิ่ม `pnlCompany` (Aggregate MT+TT+Ecom ต่อ Metric, Local ต่อไฟล์ ไม่ได้ Import จาก `pnl_executive_summary.html`) สำหรับ OPEX Ratio % KPI/Cost-to-Sales Ratio Table/Donut/Callout — Trade Spend by Partner เป็น Static Placeholder ล้วน (`renderTradeSpendByPartnerPlaceholder()`), ไม่มี `rand()`/Data Source ผูกอยู่ | Spec §5.6, `Charmiss_Module_PnL_Spec.md` |
